...
const Renderer = {
  ...
  _drawPitfall(ctx, x, y, cellPx) {
    ...
    // ポップな8bit「データホール」。普通の穴ではなく、仮想床がバグで欠損した危険マス。
    // 判定・座標・サイズはそのまま、見た目だけを電子空間の崩壊表現へ寄せる。
    ctx.shadowColor = "rgba(77,238,234,0.55)";
    ctx.shadowBlur = Math.max(4, cellPx * 0.15);
    R(4, 3, 8, 10, "rgba(77,238,234,0.10)");
    R(3, 5, 10, 6, "rgba(143,95,255,0.12)");
    ctx.shadowBlur = 0;

    // ひび割れた仮想床プレート（背景床より少し暗い程度で、初見殺しになりすぎない）
    R(3, 3, 10, 10, "#0c1428");
    R(4, 4, 8, 8, "#111d38");
    R(5, 5, 6, 6, "#080d20");

    // 中央へ吸い込まれる暗いバグホール。黒ベタにせず青紫を残して電子感を出す。
    R(6, 5, 4, 1, "#26346d");
    R(5, 6, 6, 1, "#182456");
    R(5, 7, 6, 2, "#070a18");
    R(6, 9, 4, 1, "#0c1230");
    ...
    // シアン×紫のネオン縁取り。データ欠損の輪郭として小さくても認識しやすくする。
    R(5, 3, 3, 1, "#4deeea");
    R(9, 3, 2, 1, "#b991ff");
    R(3, 5, 1, 3, "#4deeea");
    R(12, 6, 1, 3, "#8a56ff");
    ...
    // 中央の吸い込み渦。少ないドットで「電子空間へ落ちる」印象を作る。
    R(7, 6, 3, 1, "#b991ff");
    R(9, 7, 1, 1, "#4deeea");
    R(6, 8, 4, 1, "#6a3cff");
    R(6, 9, 1, 1, "#4deeea");
    ...
    // グリッチ破片。可愛くポップに見える範囲で、危険なバグ感を追加する。
    R(2, 4, 1, 1, "#4deeea");
    R(13, 4, 1, 1, "#ff2e6d");
    R(2, 9, 2, 1, "#8a56ff");
    R(12, 10, 2, 1, "#4deeea");
    ...
    // 床データの細い亀裂。普通の穴ではなく「壊れた床」として読めるようにする。
    R(4, 7, 1, 1, "#26346d");
    R(5, 8, 1, 1, "#26346d");
    R(10, 6, 1, 1, "#26346d");
    R(11, 5, 1, 1, "#26346d");
